...
from .const import (
    ...
    OPT_AIR_RECOVERY, OPT_AQI_THRESHOLD, OPT_AUTO_APPLY, OPT_FILTER_LIFE, OPT_PORTABLE_AC, OPT_VENT_REVERT, OPT_CO2_THRESHOLD, OPT_CO2_VENTILATE, OPT_COMPRESSOR_MIN_CYCLE, OPT_DEVICE_MIN_CYCLE, OPT_QUIET_HOURS, OPT_QUIET_START, OPT_QUIET_END, OPT_QUIET_MAX_TEMP, OPT_IONIZER_MODE, OPT_HUMIDITY_SENSITIVITY, OPT_AWAY_MAX_DRIFT, OPT_COIL_DRY_OUT,
...
_GLOBAL_NUMBER_OPTIONS = (
    ...
    (OPT_LIGHTNING_DISTANCE, dict(min=1, max=40, step=1, unit_of_measurement="km")),
)
_ROOM_NUMBER_OPTIONS = (
    ...
    (OPT_AWAY_MAX_DRIFT, dict(min=0, max=10, step=1, unit_of_measurement="°C")),
    (OPT_COIL_DRY_OUT, dict(min=0, max=600, step=30, unit_of_measurement="s")),
...
class EnvironmentEngineOptionsFlow(config_entries.OptionsFlow):
    async def async_step_init(self, user_input=None):
        ...
        if self.config_entry.data.get(CONF_ENTRY_TYPE) == ENTRY_GLOBAL:
            ...
            schema = _options_schema(_ROOM_NUMBER_OPTIONS, current, booleans=(OPT_AUTO_APPLY, OPT_FAN_COMFORT, OPT_PORTABLE_AC, OPT_QUIET_HOURS), times=(OPT_QUIET_START, OPT_QUIET_END), selects=((OPT_IONIZER_MODE, (IONIZER_WITH_PURIFIER, IONIZER_SURGE, IONIZER_NEVER)), (OPT_HUMIDITY_SENSITIVITY, HUMIDITY_SENSITIVITY_LEVELS)))
